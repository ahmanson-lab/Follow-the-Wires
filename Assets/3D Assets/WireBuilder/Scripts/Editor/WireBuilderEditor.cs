    ...
    public class WireBuilderEditor : Editor
    {
        ...
        private static void CreateClusterPreview(WireConnectorGroup sourceCluster)
        {
            ...
            if (sourceCluster)
            {
                ...
                {
                    if (sourceCluster.connectors[i] == null)
                    {
                        Debug.LogError("Connector group has a null connector object, this should be removed through the inspector", sourceCluster.gameObject);
                        continue;;
                    }
